...
namespace Asterra.Gameplay
{
    ...
    public static class ContentAndEconomySelfTest
    {
        public static string Run()
        {
            ...
            Expect(ref fails, sb, "six factions registered", SixFactions());
        ...
        private static bool SixFactions()
        {
            var all = FactionDefaultContent.All;
            return all != null && all.Length >= 6;
        }
        ...
        private static bool MeleeDamagesEnemy()
        {
            ...
            var a = sim.SpawnUnit(ids.Next(), p0, new FactionId(0), FactionDefaultContent.VeiledApprenticeId, 0f, 0f);
            var b = sim.SpawnUnit(ids.Next(), p1, new FactionId(1), FactionDefaultContent.RoyalPeasantId, 3f, 0f);
            float hp = b.Health;
        ...
        private static bool TowerAttacksEnemy()
        {
            ...
            var enemy = sim.SpawnUnit(
                ids.Next(), p1, new FactionId(1), FactionDefaultContent.RoyalPeasantId, 12f, 0f);
        ...
        private static bool CancelProductionRefunds()
        {
            ...
            wallet.Seed(p, ResourceType.Gold, 500);
            var keep = sim.SpawnBuilding(
                ids.Next(), p, new FactionId(0), FactionDefaultContent.ArcaneumId, 20f, 20f, startActive: true);
            if (!defs.TryGetUnit(FactionDefaultContent.VeiledBuilderId, out var unitDef))
                return false;

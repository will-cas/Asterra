"""Export upgraded Mundor Legion / Guard / Longbow meshes + stills to /tmp then copy."""
...
TMP = Path("/tmp/asterra_infantry_stills")
ROSTER = ["unit_royal_legion", "unit_royal_guard", "unit_royal_longbow"]
...
def main():
    ...
    review.configure_render(scene, samples=28, size=(1280, 720))

    for def_id in ROSTER:
        wipe({ground.name})
        print("start", def_id, flush=True)
        ob = units.UNITS[def_id](g, m, asterra_roster._coll_for(g, def_id))
        ob.location = (0, 0, 0)
    ...
    print("mundor infantry export complete", flush=True)
